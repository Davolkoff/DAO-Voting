...
describe("Vote token functions", function () {
  ...
  describe("Deploying", function () {
    it("Should deploy ERC20 contract and mint them on your balance", async function () {
      ...
      const token20 = await ethers.getContractFactory("VoteToken");
  ...
  describe("Transfers", function(){
    ...
    it("Should revert minting if you are not an owner", async function() {
      await expect(contract.connect(addr2).mint(addr2.address, 10)).to.be.revertedWith("Only for owner or voting contract");
    });

    it("Should revert burning if you are not an owner", async function() {
      await expect(contract.connect(addr2).burn(addr2.address, 10)).to.be.revertedWith("Only for owner or voting contract");
    });
  });
});