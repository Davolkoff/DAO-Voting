...
contract VoteToken {
    ...
    address private _vc; // address of a voting contract
    bool public vcConnected; // is voting contract connected or not

    modifier requireOwnerOrVC {
        require(msg.sender == _owner || msg.sender == _vc, "Only for owner or voting contract");
        _;
    }
    ...
    // connects DAO voting to ERC20
    function connectVc (address vc_) public requireOwnerOrVC {
        vcConnected = true;
        _vc = vc_;
    }

    // deletes tokens from system
    function burn(address account, uint256 amount) public requireOwnerOrVC {
    ...
    function mint(address account, uint256 amount) public requireOwnerOrVC {
        _totalSupply += amount;
        _balances[account] += amount;
